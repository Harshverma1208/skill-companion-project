...
const ChartsSection = ({ selectedTab, marketData, loading, error }) => {
  ...
  if (error && (!marketData || Object.values(marketData).every(item => !item))) {
    return (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie
                ...
                fill="#8884d8"
                dataKey="value"
                label={({ name, value, jobs }) => `${name} (${value}%) - ${jobs.toLocaleString()} jobs`}
              >
  ...
  if (selectedTab === 'TRENDING TECHNOLOGIES' && (!marketData.trendingTech || marketData.trendingTech.length === 0)) {
    return (
      <Typography variant="body1" color="text.secondary" align="center">
...
function TrendsWithGemini() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setError('Some data may not be up to date. Showing most recent available data.');
  ...
  const isCurrentTabLoading = () => {
    ...
    if (selectedTab === 'INDUSTRY DISTRIBUTION') return loading.industryDistribution;
    return false;
  };
